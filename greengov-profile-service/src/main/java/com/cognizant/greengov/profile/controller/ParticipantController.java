...
public class ParticipantController {
	...
	public ResponseEntity<Void> updateDocumentStatus(@PathVariable Long participantId, @PathVariable Long documentId,
			@Valid @RequestBody VerificationStatusUpdateDto statusDto) {
		log.info("Received request to update document ID: {} status to {} for profile ID: {}", documentId,
				statusDto.getStatus(), participantId);
	...
	@GetMapping("/lookup")
	ResponseEntity<List<ParticipantBasicDTO>> getParticipantsLookup() {
		return ResponseEntity.ok(participantService.getAllParticipantBasics());
	}

	@GetMapping
	public ResponseEntity<List<EntityProfileResponseDto>> getAllParticipants() {
		log.info("Received request to fetch all participants for the compliance dashboard");
