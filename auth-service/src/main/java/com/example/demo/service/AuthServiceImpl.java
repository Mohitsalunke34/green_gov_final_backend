...
public class AuthServiceImpl implements AuthService {
	...
	public AuthServiceImpl(UserAccountRepo userRepo, AdminRepo adminRepo, PasswordEncoder encoder,
			JwtService jwtService, OfficerProfileRepo officerRepo, EmailService emailService) {
	...
	// For OTP sending functionality
 
	private Map<String, String> otpStore = new java.util.concurrent.ConcurrentHashMap<>();// I am going with in-memory
																							// storage rather than a
																							// different table in db
 
	// send OTP
	@Override
	public void sendForgotPasswordOtp(String email) {
 
		UserAccount user = userRepo.findByEmail(email).orElseThrow(() -> new RuntimeException("User not found"));
	...
	public void resetPassword(String email, String newPassword) {
		...
		otpStore.remove(email); // remove after success from con-hash-map
